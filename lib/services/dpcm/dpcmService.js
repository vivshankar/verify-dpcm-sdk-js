  ...
  async processConsentMetadata(itemFilter, response) {
    ...
    for (const consentID in response.consents) {
      ...
      if (!records[name] || records[name] == null) {
        const allValuesName =
          `${consent.purposeId}/${attrId}.${accessTypeId}#`;
        if (records[allValuesName] && records[allValuesName] != null) {
          records[name] = { ...records[allValuesName] };
          records[name].attributeValue = attrValue;
          metadata.default.push(records[name]);
        } else {
          continue;
        }
      }
