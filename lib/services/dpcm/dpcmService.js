...
class DPCMService extends Service {
    ...
    */
    async requestApproval(duaRequest) {
        const response = await this.post('/v1.0/privacy/data-usage-approval',
            {
                trace: this._context.trace,
                subjectId: this._context.subjectId,
                isExternalSubject: this._context.isExternalSubject,
                geoIP: this._context.ipAddress,
                items: duaRequest,
            });
    ...
    */
    async getConsentMetadata(purposes) {
        const response = await this.post('/v1.0/privacy/data-subject-presentation',
            {
                subjectId: super._context.subjectId,
                isExternalSubject: super._context.isExternalSubject,
                geoIP: super._context.ipAddress,
                purposeId: purposes,
            });
        return response.data;
    }
