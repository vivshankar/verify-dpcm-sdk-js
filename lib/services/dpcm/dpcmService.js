...
class DPCMService extends Service {
            ...
            if (purpose.attributes && Array.isArray(purpose.attributes)) {
                for (const attribute of purpose.attributes) {
                    ...
                    for (const accessType of attribute.accessTypes) {
                        ...
                        let record = await this._buildMetadataRecord(response, purpose, attributeID, accessType.id, accessType.assentUIDefault);
                        records[name] = record;
                ...
                for (const accessType of purpose.accessTypes) {
                    ...
                    let record = await this._buildMetadataRecord(response, purpose, null, accessType.id, false);
                    records[name] = record;
                    if (purpose.category === "eula") {
                        metadata.eula.push(record);
                    } else {
    ...
    */
    ...
    async _buildMetadataRecord(response, purpose, attributeID, accessTypeID, assentUIDefault) {
        return {
