  ...
  async assess(items) {
    ...
    try {
      ...
      for (const ia of assessment) {
        ...
        if (ia.result &&
            (// No consent records that match the request criteria were found.
             ia.result[0].reason.messageId === 'CSIBT0033I' ||
             // A matching consent record does not exist. The request was completed 
             // because rule allows implicit assent.
             ia.result[0].reason.messageId === 'CSIBT0034I' ||
             // The consent record has expired.
             ia.result[0].reason.messageId === 'CSIBT0044I' ||
             // No consent record exists for the current active version of purpose.
             (!ia.result[0].approved && ia.result[0].reason.messageId === 'CSIBT0052I') ||
             // A matching consent record is associated with an older version of the 
             // purpose. The request was completed because rule allows implicit assent
             (!ia.result[0].approved && ia.result[0].reason.messageId === 'CSIBT0065I'))) {
