...
const GlobalAttributeMapping = require('./services/dpcm/globalAttributeMapper')
...
class Privacy {
    ...
    async getConsentMetadata(items) {
        ...
        try {
            ...
            for (const item of items) {
                ...
                let attrId = item['attributeId'];
                if (attrId && attrId != null && GlobalAttributeMapping[attrId]) {
                    item['attributeId'] = GlobalAttributeMapping[item['attributeId']];
                }

                itemNameSet.add(
                    `${item.purposeId}/${StringUtils.getOrDefault(item, 'attributeId', "")}.${StringUtils.getOrDefault(item, 'accessTypeId', "")}`);
            }
    ...
    * <br><br>Consents may only be created typically, except if the consent end time needs to
    * be updated. Only 10 consent operations are allowed at a time.
    * 
    * @param {Consent[]} consents The full consent records that need to be created or updated
    * 
    * @return {Promise<Object>} Consent operation response
    * @example
    * let r = await client.storeUserConsents([
    *   {
    *     "purposeId": "marketing",
    ...
    */
     async storeConsents(consents) {
        ...
        try {
            ...
            // parse the response
            let status = (r.messageId != "CSIBT0070I") ? "fail" : "success";
